...
// Токен
  const TOKEN = '5456338334:AAFcAYm9x82jTNx7qDETrN3n1xcMRrsF4xw';

  // ID чата
  const CHATID = '-476834460';
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  ...
  if (!empty($_POST['name']) && !empty($_POST['telephone']) !empty($_POST['email']) !empty($_POST['country']) !empty($_POST['instagramm'])) {
    ...
    if (isset($_POST['telephone']) && !empty($_POST['telephone'])) {
        $txt .= "Телефон: " . strip_tags(trim(urlencode($_POST['telephone']))) . "%0A";
    }

    //емейл
    if (isset($_POST['email']) && !empty($_POST['email'])) {
      $txt .= "Емейл: " . strip_tags(trim(urlencode($_POST['email']))) . "%0A";
  }

    //емейл
    if (isset($_POST['country']) && !empty($_POST['country'])) {
      $txt .= "Страна " . strip_tags(trim(urlencode($_POST['country']))) . "%0A";
    }
    //емейл
    if (isset($_POST['Instagramm']) && !empty($_POST['Instagramm'])) {
      $txt .= "Instagramm " . strip_tags(trim(urlencode($_POST['Instagramm']))) . "%0A";
    }
